...
|------|-------|--------|--------|-------|-----------|
| [2026-08-20T23:52:00](2026-08-20T23-52-00.md) | Game | ✅ PASS | 29 | 29 | `roll-state-machine` — roll/reveal/celebrate lifecycle refactored from 9 state-bag flags into one discriminated `state.phase` machine (`roll-phase.js`). All six phases exercised on both clients incl. the rare `awaitingServer`. **60 roll taps through an open overlay left the phase untouched**; overlays 2997–3025ms across 7 rounds with perfect winner/loser complementarity; pause-mid-reveal stashed and routed correctly; 54/54 pixel baselines at maxDiffPixels:0. Steps 30–34 skipped (would rebuild the live prod stack). 🔴 **The pixel harness's `browser-guard.js` bare `docker compose up -d web` took prod down for 14 min — still armed** |
| [2026-07-08T07:24:00](2026-07-08T07-24-00.md) | Game | ✅ PASS | 34 | 34 | `claude/multiplayer-game-discovery-gps-ttcma6` post dead-code-sweep pass. **Inline QR stamp verified** (`.qr` is a `data:` URL, no fetch). Skill self-updated for the lobby-stamp + unified-auth (`#auth-submit-btn`) reworks + others-only roster + `lower(username)` SQL. Overlay consistency 2988–3041ms across 7 rounds, no flash. Env gotchas only: `WEBAUTHN_RP_ID=localhost` override, `.env.prod` inline secrets, dev↔prod `pg_data` password mismatch needing `down -v` both ways |
