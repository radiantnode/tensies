...
@app.get("/stats/player/{user_id}")
async def stats_player(user_id: str):
    """Lifetime stats for one player, drawn from the player_stats rollup."""
...
@app.get("/stats/game/{game_code}")
async def stats_game(game_code: str):
    """Summary of a single game with per-round and per-player breakdowns."""
    from telemetry import store
...
async def stats_leaderboard(limit: int = 25):
